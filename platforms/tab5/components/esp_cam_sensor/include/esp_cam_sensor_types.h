...
#define ESP_CAM_SENSOR_NULL_POINTER_CHECK(tag, p) \
    ESP_RETURN_ON_FALSE((p), ESP_ERR_INVALID_ARG, tag, "input parameter '" #p "' is NULL")
...
#define ESP_CAM_SENSOR_CLASS_ID(class, id)     \
    (((class) << ESP_CAM_SENSOR_CLASS_SHIFT) | \
     ((id) << ESP_CAM_SENSOR_ID_SHIFT)) /*!< Transform camera sensor class and ID to uint32_t type value */
#define ESP_CAM_SENSOR_CID_GET_CLASS(val) \
    (((val) >> ESP_CAM_SENSOR_CLASS_SHIFT) & 0xff) /*!< Get camera sensor class from uint32_t type value */
#define ESP_CAM_SENSOR_CID_GET_ID(val) \
    (((val) >> ESP_CAM_SENSOR_ID_SHIFT) & 0xffff) /*!< Get camera sensor class ID from uint32_t type value */

#define ESP_CAM_SENSOR_IOC_ID_SHIFT      0
#define ESP_CAM_SENSOR_IOC_ARG_LEN_SHIFT 16
#define ESP_CAM_SENSOR_IOC(cmd, len) \
    (((cmd) << ESP_CAM_SENSOR_IOC_ID_SHIFT) | ((len) << ESP_CAM_SENSOR_IOC_ARG_LEN_SHIFT))
 ...
 */
#define ESP_CAM_SENSOR_POWER \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x01) /*!< Controller camera sensor power */
#define ESP_CAM_SENSOR_XCLK                                   \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, \
                            0x02) /*!< For sensors that require a clock provided by the base board */
#define ESP_CAM_SENSOR_SENSOR_MODE \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x03) /*!< Night mode, sunshine mode, etc */
#define ESP_CAM_SENSOR_FPS                                    \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, \
                            0x04) /*!< Frame rate output by the sensor(Frames per second) */
...
#define ESP_CAM_SENSOR_DPC \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x0e) /*!< Dead Pixel Correction from the sensor */
#define ESP_CAM_SENSOR_JPEG_QUALITY ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x0f)
#define ESP_CAM_SENSOR_BLC \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x10) /*!< Black level Correction from the sensor */
#define ESP_CAM_SENSOR_SPECIAL_EFFECT ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x11)
#define ESP_CAM_SENSOR_LENC \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_DEFAULT, 0x12) /*!< Len Shading Correction from the sensor */
...
#define ESP_CAM_SENSOR_EXPOSURE_VAL \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x02)                           /*!< Exposure target value */
...
#define ESP_CAM_SENSOR_AE_CONTROL \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x05) /*!< Automatic exposure control */
...
#define ESP_CAM_SENSOR_AE_LEVEL \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x16) /*!< Automatic exposure level */
#define ESP_CAM_SENSOR_GAIN \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x17) /*!< Absolute gain (analog gain + digital gain) */
#define ESP_CAM_SENSOR_STATS \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x18) /*!< Camera sensor gain & wb statistical data */
#define ESP_CAM_SENSOR_AE_FLICKER \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, 0x19) /*!< Anti banding flicker */
#define ESP_CAM_SENSOR_AUTO_N_PRESET_WB                  \
    ESP_CAM_SENSOR_CLASS_ID(ESP_CAM_SENSOR_CID_CLASS_3A, \
                            0x20) /*!< Pre set white balance mode when automatic white balance is not enabled */
...
typedef struct _cam_sensor_format_struct {
    ...
    const esp_cam_sensor_isp_info_t
        *isp_info; /*!< For sensor without internal ISP, set NULL if the sensor‘s internal ISP used. */
    esp_cam_sensor_mipi_info_t mipi_info; /*!< MIPI RX init cfg */
    void *
        reserved; /*!< can be used to provide AE\AF\AWB info or Parameters of some related accessories（VCM、LED、IR）*/
...
typedef struct {
    esp_sccb_io_handle_t
        sccb_handle;      /*!< the handle of the sccb bus bound to the sensor, returned by sccb_new_i2c_io */
...
typedef struct {
    union {
        ...
        esp_cam_sensor_device_t *(*fn)(void *)
            __attribute__((deprecated("please use detect instead"))); /*!< Pointer to the detect function */
    };

 ...
 * Note2: the analog gain of sc2336 will be affected by temperature, it is recommended to increase Dgain first and then
 * Again.
 */
typedef struct {
...
static esp_err_t sc2336_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint16_t reg, uint8_t offset, uint8_t length,
                                     uint8_t value)
{
    esp_err_t ret    = ESP_OK;
    uint8_t reg_data = 0;
...
static esp_err_t sc2336_set_para_value(esp_cam_sensor_device_t *dev, uint32_t id, const void *arg, size_t size)
{
    ...
    switch (id) {
        case ESP_CAM_SENSOR_EXPOSURE_VAL: {
            ...
            ret = sc2336_write(dev->sccb_handle, SC2336_REG_SHUTTER_TIME_H, SC2336_FETCH_EXP_H(u32_val));
            ret |= sc2336_write(dev->sccb_handle, SC2336_REG_SHUTTER_TIME_M, SC2336_FETCH_EXP_M(u32_val));
            ret |= sc2336_write(dev->sccb_handle, SC2336_REG_SHUTTER_TIME_L, SC2336_FETCH_EXP_L(u32_val));
            if (ret == ESP_OK) {
        ...
        case ESP_CAM_SENSOR_GAIN: {
            ESP_LOGD(TAG, "dgain_fine %" PRIx8 ", dgain_coarse %" PRIx8 ", again_coarse %" PRIx8,
                     sc2336_gain_map[u32_val].dgain_fine, sc2336_gain_map[u32_val].dgain_coarse,
                     sc2336_gain_map[u32_val].analog_gain);
            ret = sc2336_write(dev->sccb_handle, SC2336_REG_DIG_FINE_GAIN, sc2336_gain_map[u32_val].dgain_fine);
            ret |= sc2336_write(dev->sccb_handle, SC2336_REG_DIG_COARSE_GAIN, sc2336_gain_map[u32_val].dgain_coarse);
            ret |= sc2336_write(dev->sccb_handle, SC2336_REG_ANG_GAIN, sc2336_gain_map[u32_val].analog_gain);
            if (ret == ESP_OK) {
...
static const esp_cam_sensor_ops_t sc2336_ops = {.query_para_desc          = sc2336_query_para_desc,

...
static const esp_cam_sensor_isp_info_t sc035hgs_isp_info[] = {{.isp_v1_info =
                                                                   {
...
static esp_err_t sc035hgs_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint16_t reg, uint8_t offset, uint8_t length,
                                       uint8_t value)
{
...
static esp_err_t sc035hgs_set_para_value(esp_cam_sensor_device_t *dev, uint32_t id, const void *arg, size_t size)
{
    ...
    switch (id) {
        case ESP_CAM_SENSOR_EXPOSURE_VAL: {
            ...
            // Todo, convert to uint seconds, sc035hgs exposure time use 1/16 * LTime as step, and EXP Rows = {0x3e01,
            // 0x3e02} + 0x3226
        ...
        case ESP_CAM_SENSOR_GAIN: {
            uint32_t u32_val = *(uint32_t *)arg;
            ESP_LOGD(TAG, "again_fine %" PRIx8 ", again_coarse %" PRIx8 ", dgain_fine %" PRIx8 ", dgain_coarse %" PRIx8,
                     sc035hgs_gain_map[u32_val].again_fine, sc035hgs_gain_map[u32_val].again_coarse,
                     sc035hgs_gain_map[u32_val].dgain_fine, sc035hgs_gain_map[u32_val].dgain_coarse);

            ret = sc035hgs_set_reg_bits(dev->sccb_handle, SC035HGS_REG_FINE_AGAIN, 2, 3,
                                        sc035hgs_gain_map[u32_val].again_fine);
            ret |= sc035hgs_write(dev->sccb_handle, SC035HGS_REG_COARSE_AGAIN, sc035hgs_gain_map[u32_val].again_coarse);
            ret |= sc035hgs_set_reg_bits(dev->sccb_handle, SC035HGS_REG_FINE_DGAIN, 0, 2,
                                         sc035hgs_gain_map[u32_val].dgain_fine);
            ret |= sc035hgs_write(dev->sccb_handle, SC035HGS_REG_COARSE_DGAIN, sc035hgs_gain_map[u32_val].dgain_coarse);
            if (ret == ESP_OK) {
...
static esp_err_t sc035hgs_query_support_capability(esp_cam_sensor_device_t *dev,
...
static const esp_cam_sensor_ops_t sc035hgs_ops = {.query_para_desc          = sc035hgs_query_para_desc,

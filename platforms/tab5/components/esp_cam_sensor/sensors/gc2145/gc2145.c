...
static esp_err_t gc2145_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint8_t reg, uint8_t offset, uint8_t mask,
                                     uint8_t value)
{
    esp_err_t ret = ESP_OK;
...
static esp_err_t gc2145_set_antibanding(esp_cam_sensor_device_t *dev, int val)
{
    ...
    ret = gc2145_write_array(dev->sccb_handle, (gc2145_reginfo_t *)gc2145_antibanding[banding_array_index],
                             GC2145_ANTI_BANDING_REG_SIZE);
...
static const esp_cam_sensor_ops_t gc2145_ops = {.query_para_desc          = gc2145_query_para_desc,

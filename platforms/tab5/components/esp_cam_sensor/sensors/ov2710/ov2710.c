...
static esp_err_t ov2710_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint16_t reg, uint8_t offset, uint8_t length,
                                     uint8_t value)
{
    esp_err_t ret    = ESP_OK;
    uint8_t reg_data = 0;
...
static void wb_timer_callback(TimerHandle_t timer)
{
    ...
    cam_ov2710->ov2710_para.stats.agc_gain         = ((read_v[3] & 0x01) + 1) * ((read_v[4] & 0x80) + 1) *
                                             ((read_v[4] & 0x40) + 1) * ((read_v[4] & 0x20) + 1) *
                                             ((read_v[4] & 0x10) + 1) * ((read_v[4] & 0x0f) / 16 + 1);
...
static const esp_cam_sensor_ops_t ov2710_ops = {.query_para_desc          = ov2710_query_para_desc,
...
esp_cam_sensor_device_t *ov2710_detect(esp_cam_sensor_config_t *config)
{
    ...
    cam_ov2710->wb_timer_handle = xTimerCreate("wb_t", CONFIG_CAMERA_OV2710_STATS_UPDATE_INTERVAL / portTICK_PERIOD_MS,
                                               pdTRUE, (void *)dev, wb_timer_callback);

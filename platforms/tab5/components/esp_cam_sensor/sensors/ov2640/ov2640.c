...
static esp_err_t ov2640_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint8_t bank, uint8_t reg, uint8_t offset,
...
static esp_err_t ov2640_write_reg_bits(esp_sccb_io_handle_t sccb_handle, uint8_t bank, uint8_t reg, uint8_t mask,
                                       int enable)
{
...
#define WRITE_REG_OR_RETURN(bank, reg, val)                   \
    ret = ov2640_write_reg(dev->sccb_handle, bank, reg, val); \
    if (ret) {                                                \
        return ret;                                           \
    }
#define SET_REG_BITS_OR_RETURN(bank, reg, offset, mask, val)                   \
    ret = ov2640_set_reg_bits(dev->sccb_handle, bank, reg, offset, mask, val); \
    if (ret) {                                                                 \
        return ret;                                                            \
    }
...
static esp_cam_sensor_ops_t ov2640_ops = {.query_para_desc          = ov2640_query_para_desc,

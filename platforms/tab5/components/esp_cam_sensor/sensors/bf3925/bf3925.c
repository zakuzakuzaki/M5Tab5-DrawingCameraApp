...
static esp_err_t bf3925_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint8_t reg, uint8_t offset, uint8_t length,
                                     uint8_t value)
{
    esp_err_t ret    = ESP_OK;
    uint8_t reg_data = 0;
...
static esp_err_t bf3925_set_antibanding(esp_cam_sensor_device_t *dev, int val)
{
    ...
    switch (val) {
        case BF3925_BANDING_50HZ:
            ret |=
                bf3925_write(dev->sccb_handle, 0x07,
                             0x92);  // Bit[3:2]: the bigger, Y_AVER_MODIFY is smaller,BIT[1]:banding filter selection
            ret |= bf3925_write(dev->sccb_handle, 0x0c, 0x81);  // 50hz banding 0x78
            break;
        case BF3925_BANDING_60HZ:
            ret |=
                bf3925_write(dev->sccb_handle, 0x07,
                             0x90);  // Bit[3:2]: the bigger, Y_AVER_MODIFY is smaller,BIT[1]:banding filter selection
...
static esp_err_t bf3925_set_effect(esp_cam_sensor_device_t *dev, int val)
{
    ...
    ret = bf3925_write_array(dev->sccb_handle, (bf3925_reginfo_t *)bf3925_spec_effect_regs[effect_array_index],
                             BF3925_SPEC_EFFECT_REG_SIZE);
...
static esp_err_t bf3925_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    // bf3925_write(dev->sccb_handle, BF3925_REG_SOFTWARE_STANDBY, 0x01);//Bit[0]: 1-soft reset     bit[1]:1-soft sleep
    // mode delay_ms(50);
...
static const esp_cam_sensor_ops_t bf3925_ops = {.query_para_desc          = bf3925_query_para_desc,

...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (1) {
        ...
        if (wc->pixel_format == V4L2_PIX_FMT_JPEG) {
            ...
            res = jpeg_encoder_process(wc->jpeg_handle, &wc->jpeg_enc_config, wc->buffer[buf.index], buf.bytesused,
                                       wc->jpeg_out_buf, wc->jpeg_enc_output_buf_alloced_size, &jpeg_encoded_size);
            if (res == ESP_OK) {
...
static esp_err_t new_web_cam(int cam_fd, web_cam_t **ret_wc)
{
    ...
    wc->jpeg_out_buf = (uint8_t *)jpeg_alloc_encoder_mem(jpeg_enc_input_src_size / 2, &jpeg_enc_output_mem_cfg,
...
static esp_err_t http_server_init(int index, web_cam_t *web_cam)
{
    ...
    httpd_uri_t stream_get_uri = {
        .uri = "/stream", .method = HTTP_GET, .handler = stream_handler, .user_ctx = (void *)web_cam};
...
static void initialise_mdns(void)
{
    ...
    mdns_txt_item_t serviceTxtData[] = {{"board", CONFIG_IDF_TARGET}, {"path", "/"}};

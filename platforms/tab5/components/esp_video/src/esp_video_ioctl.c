...
static esp_err_t esp_video_ioctl_reqbufs(struct esp_video *video, struct v4l2_requestbuffers *req_bufs)
{
    ...
    if ((req_bufs->memory != V4L2_MEMORY_MMAP) && (req_bufs->memory != V4L2_MEMORY_USERPTR)) {
...
static esp_err_t esp_video_ioctl_mmap(struct esp_video *video, struct esp_video_ioctl_mmap *ioctl_mmap)
{
    ...
    if ((info.memory_type != V4L2_MEMORY_MMAP) || (ioctl_mmap->length > info.size) || (index >= info.count)) {
        return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t esp_video_ioctl_qbuf(struct esp_video *video, struct v4l2_buffer *vbuf)
{
    ...
    if (info.memory_type == V4L2_MEMORY_MMAP) {
        ...
        ret = esp_video_queue_element_index_buffer(video, vbuf->type, vbuf->index, (uint8_t *)vbuf->m.userptr,
                                                   vbuf->length);
    }
...
static inline esp_err_t esp_video_ioctl_set_sensor_format(struct esp_video *video,

 ...
 * @param ioctl    Sensor command type pointe, true: it is sensor ioctl command; false: it is sensor parameter ID
 * command
 *
 * @return
 *      - ESP_OK on success
            ...
            switch (qdesc.type) {
                case ESP_CAM_SENSOR_PARAM_TYPE_NUMBER:
                    if ((value_buf > qdesc.number.maximum) || (value_buf < qdesc.number.minimum) ||
                        ...
                        ESP_LOGE(TAG,
                                 "number: ctrl id=%" PRIx32 " value=%" PRIi32 " is out of range(max=%" PRIi32
                                 ", min=%" PRIi32 ", step=%" PRIu32 ")",
                                 ctrl->id, value_buf, (int32_t)qdesc.number.maximum, (int32_t)qdesc.number.minimum,
                                 (uint32_t)qdesc.number.step);
                ...
                case ESP_CAM_SENSOR_PARAM_TYPE_ENUMERATION: {
                    if (value_buf < 0 || value_buf >= qdesc.enumeration.count) {
                        ESP_LOGE(TAG,
                                 "enum: ctrl id=%" PRIx32 " value=%" PRIu32 " is out of range(from 0 to %" PRIu32 ")",

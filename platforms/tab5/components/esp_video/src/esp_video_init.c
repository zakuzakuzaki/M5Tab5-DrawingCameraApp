...
esp_err_t esp_video_init(const esp_video_init_config_t *config)
{
    ...
    for (esp_cam_sensor_detect_fn_t *p = &__esp_cam_sensor_detect_fn_array_start;
        ...
        if (p->port == ESP_CAM_SENSOR_MIPI_CSI && config->csi != NULL) {
            ...
            cfg.sccb_handle =
                create_sccb_device(sccb_mark, ESP_CAM_SENSOR_MIPI_CSI, &config->csi->sccb_config, p->sccb_addr);
            if (!cfg.sccb_handle) {
                return ESP_FAIL;
            }
        ...
        if (p->port == ESP_CAM_SENSOR_DVP && config->dvp != NULL) {
            ...
            cfg.sccb_handle =
                create_sccb_device(sccb_mark, ESP_CAM_SENSOR_DVP, &config->dvp->sccb_config, p->sccb_addr);
            if (!cfg.sccb_handle) {
                return ESP_FAIL;
            }

            cfg.reset_pin = config->dvp->reset_pin, cfg.pwdn_pin = config->dvp->pwdn_pin,
            cam_dev = (*(p->detect))((void *)&cfg);
            if (!cam_dev) {

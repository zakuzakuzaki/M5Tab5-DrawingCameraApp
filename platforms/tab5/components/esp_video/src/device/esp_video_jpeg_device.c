...
static esp_err_t jpeg_get_input_format_from_v4l2(uint32_t v4l2_format, jpeg_enc_input_format_t *src_type,
                                                 uint8_t *src_bpp, jpeg_down_sampling_type_t *sub_sample)
{
...
static esp_err_t jpeg_video_m2m_process(struct esp_video *video, uint8_t *src, uint32_t src_size, uint8_t *dst,
                                        uint32_t dst_size, uint32_t *dst_out_size)
{
    ...
    ret = jpeg_encoder_process(jpeg_video->enc_handle, &enc_config, src, src_size, dst, dst_size, &jpeg_codeced_size);
    if (ret == ESP_OK) {
...
static esp_err_t jpeg_video_set_format(struct esp_video *video, const struct v4l2_format *format)
{
    ...
    if (format->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
        ...
        if ((pix->pixelformat != V4L2_PIX_FMT_JPEG) || (width && (pix->width != width)) ||
        ...
        if ((width && (pix->width != width)) || (height && (pix->height != height))) {
            ESP_LOGE(TAG, "width or height is invalid");
            return ESP_ERR_INVALID_ARG;
        }

        ret = jpeg_get_input_format_from_v4l2(pix->pixelformat, &jpeg_video->src_type, &input_bpp,
                                              &jpeg_video->sub_sample);
        if (ret != ESP_OK) {
...
static esp_err_t jpeg_video_notify(struct esp_video *video, enum esp_video_event event, void *arg)
{
    ...
    if (event == ESP_VIDEO_M2M_TRIGGER) {
        ...
        if (type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
            ret = esp_video_m2m_process(video, V4L2_BUF_TYPE_VIDEO_OUTPUT, V4L2_BUF_TYPE_VIDEO_CAPTURE,

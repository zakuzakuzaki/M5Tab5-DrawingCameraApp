...
static const uint32_t s_isp_isp_format[] = {
    V4L2_PIX_FMT_SBGGR8, V4L2_PIX_FMT_RGB565, V4L2_PIX_FMT_RGB24, V4L2_PIX_FMT_YUV420, V4L2_PIX_FMT_YUV422P,
};
...
static esp_err_t isp_start_hist(struct isp_video *isp_video)
{
    ...
    ESP_RETURN_ON_ERROR(esp_isp_new_hist_controller(isp_video->isp_proc, &hist_config, &isp_video->hist_ctlr), TAG,
                        "failed to new histogram");

    ESP_GOTO_ON_ERROR(esp_isp_hist_register_event_callbacks(isp_video->hist_ctlr, &hist_cb, isp_video), fail_0, TAG,
                      "failed to register histogram callback");
    ESP_GOTO_ON_ERROR(esp_isp_hist_controller_enable(isp_video->hist_ctlr), fail_0, TAG, "failed to enable histogram");
    ESP_GOTO_ON_ERROR(esp_isp_hist_controller_start_continuous_statistics(isp_video->hist_ctlr), fail_1, TAG,
                      "failed to start histogram");
...
{
    ESP_RETURN_ON_ERROR(esp_isp_hist_controller_stop_continuous_statistics(isp_video->hist_ctlr), TAG,
                        "failed to stop histogram");
...
static esp_err_t isp_start_awb(struct isp_video *isp_video)
{
    ...
    ESP_RETURN_ON_ERROR(esp_isp_new_awb_controller(isp_video->isp_proc, &awb_config, &isp_video->awb_ctlr), TAG,
                        "failed to new AWB");

    ESP_GOTO_ON_ERROR(esp_isp_awb_register_event_callbacks(isp_video->awb_ctlr, &awb_cb, isp_video), fail_0, TAG,
                      "failed to register AWB callback");
    ESP_GOTO_ON_ERROR(esp_isp_awb_controller_enable(isp_video->awb_ctlr), fail_0, TAG, "failed to enable AWB");
    ESP_GOTO_ON_ERROR(esp_isp_awb_controller_start_continuous_statistics(isp_video->awb_ctlr), fail_1, TAG,
                      "failed to start AWB");
...
{
    ESP_RETURN_ON_ERROR(esp_isp_awb_controller_stop_continuous_statistics(isp_video->awb_ctlr), TAG,
                        "failed to stop AWB");
...
static esp_err_t isp_start_sharpen(struct isp_video *isp_video)
{
    ...
    ESP_RETURN_ON_ERROR(esp_isp_sharpen_configure(isp_video->isp_proc, &sharpen_config), TAG,
                        "failed to configure sharpen");
...
static esp_err_t isp_start_gamma(struct isp_video *isp_video)
{
    ...
    ESP_RETURN_ON_ERROR(esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_R, &gamma_config), TAG,
                        "failed to configure R GAMMA");
    ESP_RETURN_ON_ERROR(esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_G, &gamma_config), TAG,
                        "failed to configure G GAMMA");
    ESP_RETURN_ON_ERROR(esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_B, &gamma_config), TAG,
                        "failed to configure B GAMMA");
...
static esp_err_t isp_start_demosaic(struct isp_video *isp_video)
{
    ...
    ESP_RETURN_ON_ERROR(esp_isp_demosaic_configure(isp_video->isp_proc, &demosaic_config), TAG,
                        "failed to configure demosaic");
...
{
    ESP_RETURN_ON_ERROR(esp_isp_color_configure(isp_video->isp_proc, &isp_video->color_config), TAG,
                        "failed to configure color");
...
static esp_err_t isp_video_set_format(struct esp_video *video, const struct v4l2_format *format)
{
    ...
    if (pix->width != META_VIDEO_GET_FORMAT_WIDTH(video) || pix->height != META_VIDEO_GET_FORMAT_HEIGHT(video) ||
        ...
        switch (ctrl->id) {
            ...
            case V4L2_CID_USER_ESP_ISP_DEMOSAIC: {
                ...
                if (demosaic->enable) {
                    ...
                    if (ISP_STARTED(isp_video)) {
                        ESP_GOTO_ON_ERROR(isp_reconfigure_demosaic(isp_video), exit, TAG,
                                          "failed to reconfigure demosaic");
                    }
                } else {
                    if (ISP_STARTED(isp_video)) {
            ...
            case V4L2_CID_USER_ESP_ISP_WB: {
                ...
                if (ISP_STARTED(isp_video)) {
                    ESP_GOTO_ON_ERROR(isp_reconfigure_white_blance(isp_video), exit, TAG,
                                      "failed to reconfigure demosaic");
                }
                break;
            }
...
esp_err_t esp_video_create_isp_video_device(void)
{
    ...
    s_isp_video.video =
        esp_video_create(ISP_NAME, ESP_VIDEO_ISP1_DEVICE_ID, &s_isp_video_ops, &s_isp_video, caps, device_caps);
...
esp_err_t esp_video_isp_start_by_csi(const esp_video_csi_state_t *state, const struct v4l2_format *format)
{
    ...
    if ((format->fmt.pix.ycbcr_enc == V4L2_YCBCR_ENC_DEFAULT) || (format->fmt.pix.ycbcr_enc == V4L2_YCBCR_ENC_601)) {
...
#if CONFIG_ESP_VIDEO_ENABLE_ISP_VIDEO_DEVICE
    esp_isp_evt_cbs_t cbs = {.on_sharpen_frame_done = isp_sharpen_stats_done};

    ESP_GOTO_ON_ERROR(esp_isp_register_event_callbacks(isp_video->isp_proc, &cbs, isp_video), fail_1, TAG,
                      "failed to register sharpen callback");
#endif
...
esp_err_t esp_video_isp_stop(bool bypass)
{
    ...
    ESP_GOTO_ON_ERROR(esp_isp_register_event_callbacks(isp_video->isp_proc, &cbs, NULL), exit, TAG,
                      "failed to free ISP event");
#endif
...
esp_err_t esp_video_isp_check_format(const struct v4l2_format *format)
{
    ...
    if ((format->fmt.pix.pixelformat == V4L2_PIX_FMT_YUV420) || (format->fmt.pix.pixelformat == V4L2_PIX_FMT_YUV422P)) {
        if ((format->fmt.pix.ycbcr_enc != V4L2_YCBCR_ENC_DEFAULT) &&
            (format->fmt.pix.ycbcr_enc != V4L2_YCBCR_ENC_601) && (format->fmt.pix.ycbcr_enc != V4L2_YCBCR_ENC_709)) {
            return ESP_ERR_NOT_SUPPORTED;
        }

...
static esp_err_t dvp_get_input_frame_type(esp_cam_sensor_output_format_t sensor_format, cam_ctlr_color_t *in_color,
                                          uint32_t *v4l2_format, uint32_t *bpp)
{
...
static bool IRAM_ATTR dvp_video_on_trans_finished(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans,
                                                  void *user_data)
{
...
static bool IRAM_ATTR dvp_video_on_get_new_trans(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans,
                                                 void *user_data)
{
...
static esp_err_t dvp_video_start(struct esp_video *video, uint32_t type)
{
    ...
    esp_cam_ctlr_evt_cbs_t cam_ctrl_cbs = {.on_get_new_trans  = dvp_video_on_get_new_trans,
...
static esp_err_t dvp_video_set_format(struct esp_video *video, const struct v4l2_format *format)
{
    ...
    if (pix->width != CAPTURE_VIDEO_GET_FORMAT_WIDTH(video) || pix->height != CAPTURE_VIDEO_GET_FORMAT_HEIGHT(video) ||

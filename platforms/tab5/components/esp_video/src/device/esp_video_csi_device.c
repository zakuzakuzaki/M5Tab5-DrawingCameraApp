...
static bool IRAM_ATTR csi_video_on_trans_finished(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans,
                                                  void *user_data)
{
...
static bool IRAM_ATTR csi_video_on_get_new_trans(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans,
                                                 void *user_data)
{
    struct esp_video_buffer_element *element;
    struct esp_video *video = (struct esp_video *)user_data;
...
static esp_err_t init_config(struct esp_video *video)
{
    ...
    ESP_RETURN_ON_ERROR(csi_get_data_lane(sensor_format.mipi_info.lane_num, &csi_video->state.lane_num), TAG,
                        "failed to get CSI data lane number");
    ESP_RETURN_ON_ERROR(csi_get_input_frame_type(sensor_format.format, &csi_video->state.in_color, &csi_in_bpp), TAG,
                        "failed to get CSI input frame format");
    ...
    if (sensor_format.isp_info) {
        ...
        ESP_RETURN_ON_ERROR(v4l2_get_input_frame_type_from_sensor(sensor_format.format, &v4l2_format), TAG,
                            "failed to get V4L2 input frame type");
    ...
    CAPTURE_VIDEO_SET_FORMAT(video, sensor_format.width, sensor_format.height, v4l2_format);

    uint32_t buf_size =
        CAPTURE_VIDEO_GET_FORMAT_WIDTH(video) * CAPTURE_VIDEO_GET_FORMAT_HEIGHT(video) * csi_video->state.out_bpp / 8;
...
static esp_err_t csi_video_start(struct esp_video *video, uint32_t type)
{
    ...
    ESP_GOTO_ON_ERROR(esp_cam_ctlr_register_event_callbacks(csi_video->cam_ctrl_handle, &cam_ctrl_cbs, video), exit_0,
                      TAG, "failed to register CAM ctlr event callback");
    ...
    ESP_GOTO_ON_ERROR(esp_video_isp_start_by_csi(&csi_video->state, STREAM_FORMAT(CAPTURE_VIDEO_STREAM(video))), exit_2,
                      TAG, "failed to start ISP");

    int flags = 1;
    ESP_GOTO_ON_ERROR(esp_cam_sensor_ioctl(csi_video->cam_dev, ESP_CAM_SENSOR_IOC_S_STREAM, &flags), exit_3, TAG,
                      "failed to start sensor stream");
...
static esp_err_t csi_video_stop(struct esp_video *video, uint32_t type)
{
    ...
    int flags = 0;
    ESP_RETURN_ON_ERROR(esp_cam_sensor_ioctl(csi_video->cam_dev, ESP_CAM_SENSOR_IOC_S_STREAM, &flags), TAG,
                        "failed to stop sensor stream");
...
static esp_err_t csi_video_set_format(struct esp_video *video, const struct v4l2_format *format)
{
    ...
    if (csi_video->state.bypass_isp) {
        ...
        ESP_RETURN_ON_ERROR(esp_video_isp_check_format(format), TAG, "ISP does not support format=%" PRIx32,
                            pix->pixelformat);

        ESP_RETURN_ON_ERROR(csi_get_output_frame_type_from_v4l2(pix->pixelformat, &csi_video->state.out_color,
                                                                &csi_video->state.out_bpp),
                            TAG, "CSI does not support format=%" PRIx32, pix->pixelformat);

        uint32_t buf_size = CAPTURE_VIDEO_GET_FORMAT_WIDTH(video) * CAPTURE_VIDEO_GET_FORMAT_HEIGHT(video) *
                            csi_video->state.out_bpp / 8;

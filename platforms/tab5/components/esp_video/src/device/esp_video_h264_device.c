...
static esp_err_t h264_get_input_format_from_v4l2(uint32_t v4l2_format, esp_h264_raw_format_t *input_format,
                                                 uint8_t *input_bpp)
{
...
static esp_err_t h264_video_m2m_process(struct esp_video *video, uint8_t *src, uint32_t src_size, uint8_t *dst,
                                        uint32_t dst_size, uint32_t *dst_out_size)
{
    esp_h264_err_t h264_err;
    esp_h264_enc_in_frame_t in_frame   = {.raw_data = {
                                            .buffer = src,
                                            .len    = src_size,
                                        }};
    esp_h264_enc_out_frame_t out_frame = {.raw_data = {
                                              .buffer = dst,
                                              .len    = dst_size,
                                          }};
...
static esp_err_t h264_video_start(struct esp_video *video, uint32_t type)
{
    ...
    if (type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
        esp_h264_enc_cfg_hw_t config = {.pic_type = h264_video->input_format,
                                        .gop      = h264_video->gop,
                                        .fps      = h264_video->gop,
                                        .res =
                                            {
...
static esp_err_t h264_video_set_format(struct esp_video *video, const struct v4l2_format *format)
{
    ...
    if (format->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
        ...
        if ((pix->pixelformat != V4L2_PIX_FMT_H264) || (width && (pix->width != width)) ||

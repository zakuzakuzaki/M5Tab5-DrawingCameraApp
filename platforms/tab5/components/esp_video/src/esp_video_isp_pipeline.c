...
static void print_dev_info(int fd)
{
    ...
    ESP_LOGD(TAG, "version: %d.%d.%d", (uint16_t)(capability.version >> 16), (uint8_t)(capability.version >> 8),
...
static void config_white_balance(esp_video_isp_t *isp, esp_ipa_metadata_t *metadata)
{
    ...
    if (rc && bg) {
        esp_video_isp_wb_t wb = {.enable = true, .red_gain = metadata->red_gain, .blue_gain = metadata->blue_gain};
...
esp_err_t esp_video_isp_pipeline_init(const esp_video_isp_config_t *config)
{
    ...
    if (!config || !config->isp_dev || !config->cam_dev || !config->ipa_nums || !config->ipa_names) {
    ...
    metadata.flags = 0;
    ESP_GOTO_ON_ERROR(esp_ipa_pipeline_init(isp->ipa_pipeline, &isp->sensor, &metadata), fail_3, TAG,
                      "failed to initialize IPA pipeline");
    config_isp_and_camera(isp, &metadata);

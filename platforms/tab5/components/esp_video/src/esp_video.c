...
esp_err_t esp_video_setup_buffer(struct esp_video *video, uint32_t type, uint32_t memory_type, uint32_t count)
{
    ...
    if (!info->size || !info->align_size || !info->caps) {
        ESP_LOGE(TAG, "Failed to check buffer information: size=%" PRIu32 " align=%" PRIu32 " cap=%" PRIx32, info->size,
                 info->align_size, info->caps);
        return ESP_ERR_INVALID_STATE;
    }
 ...
 *      - Others if failed
 */
esp_err_t IRAM_ATTR esp_video_done_element(struct esp_video *video, uint32_t type,
                                           struct esp_video_buffer_element *element)
{
    struct esp_video_stream *stream;
                                               ...
                                               uint32_t size)
{
    ...
    if ((info->memory_type != V4L2_MEMORY_USERPTR) || (((uintptr_t)buffer) % info->align_size) || (size < info->size)) {
        return ESP_ERR_INVALID_ARG;
    }
                                ...
                                esp_video_m2m_process_t proc)
{
    ...
    ret = esp_video_get_m2m_queued_elements(video, src_type, &src_element, dst_type, &dst_element);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "no valid buffer");
        return ret;
    }

    ret = proc(video, ELEMENT_BUFFER(src_element), ELEMENT_SIZE(src_element), ELEMENT_BUFFER(dst_element),
               ELEMENT_SIZE(dst_element), &dst_out_size);
    if (ret != ESP_OK) {
